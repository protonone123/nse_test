...
import resample_utils     # derive 1wk/1mo locally from 1d (no extra network calls)
...
def run_eod_update(stocks: list[str], full_refresh: bool = False):
    ...
    FIX (this is the "why is it downloading weekly/monthly again"
    issue): only "1d" is ever downloaded over the network. 1wk/1mo are
    derived LOCALLY from the just-cached daily bars via
    resample_utils.resample_weekly/resample_monthly — exactly what the
    cup_test scanner's downloader.py does (it downloads daily once and
    calls resample_weekly()/resample_monthly() wherever a weekly/
    monthly DataFrame is needed; it never hits yfinance with
    interval="1wk"/"1mo" at all). The previous version of this function
    looped over TF_EOD and issued a full batched download PASS per
    timeframe — three full network passes instead of one, for data that
    Yahoo itself only ever computes by aggregating the same daily bars.

    Still batches ~50 symbols per yf.download() call for the one real
    network pass (see batch_downloader.py) — that part is unchanged.
    """
    log.info(f"EOD update (batched, download-once): {len(stocks)} stocks")
    t0 = time.time()
    totals = {}

    # ── 1. Network: daily bars only ──────────────────────────────────
    # save_df wrapped with split_guard: if a corporate action (split/
    # bonus) happened since the cache was last written, the last
    # cached close and the first newly-fetched close will be wildly
    # inconsistent (organic single-day moves never look like this) —
    # detected and fixed at the source by discarding the stale cache
    # and doing one full re-fetch for just that symbol, rather than
    # silently merging two different price scales into one series.
    def _save_1d(sym, df):
        split_guard.guarded_save(
            sym, df,
            read_existing_fn=lambda s: read_cache(s, "1d"),
            write_fn=lambda s, d: write_cache(s, "1d", d),
            interval="1d", full_history_start="2015-01-01", log=log,
        )

    stats_1d = batch_downloader.run_batch_download(
        symbols=stocks,
        get_last_date=lambda sym: get_last_date(sym, "1d"),
        save_df=_save_1d,
        interval="1d",
    ...
    totals["1d"] = stats_1d
    log.info(f"  1d: saved={stats_1d['saved']} failed={len(stats_1d['failed'])} "
             f"batches={stats_1d['batches']} ({stats_1d['elapsed_sec']:.0f}s)")

    # ── 2. Local (no network): derive 1wk/1mo from the daily cache ───
    t1 = time.time()
    wk_saved = mo_saved = skipped = 0
    for sym in stocks:
        try:
            daily = read_cache(sym, "1d")
            if daily is None or len(daily) < 5:
                skipped += 1
                continue
            weekly, monthly = resample_utils.derive_weekly_monthly(daily)
            if weekly is not None and not weekly.empty:
                write_cache(sym, "1wk", weekly)
                wk_saved += 1
            if monthly is not None and not monthly.empty:
                write_cache(sym, "1mo", monthly)
                mo_saved += 1
        except Exception as e:
            log.debug(f"resample {sym}: {e}")
    resample_elapsed = time.time() - t1
    totals["1wk"] = {"saved": wk_saved, "failed": [], "batches": 0, "elapsed_sec": round(resample_elapsed, 1)}
    totals["1mo"] = {"saved": mo_saved, "failed": [], "batches": 0, "elapsed_sec": round(resample_elapsed, 1)}
    log.info(f"  1wk: derived locally for {wk_saved}/{len(stocks)} stocks "
             f"({resample_elapsed:.1f}s, no network)")
    log.info(f"  1mo: derived locally for {mo_saved}/{len(stocks)} stocks "
             f"({resample_elapsed:.1f}s, no network)")
    if skipped:
        log.info(f"  {skipped} stocks skipped resample (insufficient daily history in cache)")

    elapsed = time.time() - t0
    total_saved = stats_1d["saved"] + wk_saved + mo_saved
    total_failed = len(stats_1d["failed"])
